...
function run_simulation(config::Dict, output_path::String)
    ...
    n = Int(config["numerics"]["n"])                        # 2D resolution = n^2
    stepper = String(config["numerics"]["stepper"])         # timestepper
    dt = Float64(config["numerics"]["dt"])                  # timestep
    nsteps = Int(config["numerics"]["nsteps"])               # total number of time-steps
    save_substeps = Int(config["numerics"]["save_substeps"]) # number of timesteps after which output is saved
    ...
    plotname = "snapshots"
    # Intermediate JLD2 output derived from output_path (unique per sample), not
    # config's jld2_filename, so multiple samples run in one process never collide.
    filepath = splitext(output_path)[1] * ".jld2"
    mkpath(dirname(output_path)) # output_path's directory may not exist yet; filepath needs it too
    ...
    rm(filepath; force=true) # clean up the per-sample intermediate JLD2 file
...
function main()
    ...
    output_path = joinpath(".", String(config["output"]["nc_filename"]))
